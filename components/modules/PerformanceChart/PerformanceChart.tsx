...
export default function PerformanceChart({
  ...
  const getBreakdownData = (data: any, type: string): Entry[] => {
    return ['Live', 'Video', 'Product Card'].map((name, index) => ({
      ...
      value: parseFloat(data?.[type]?.[index]?.amount || '0')
    }));
  };
  ...
  const PieChartComponent = ({
    ...
    <div className="w-80 max-w-full">
      <h4 className="text-lg font-semibold text-center text-navy-200">
        {title}
      </h4>
  ...
  return (
    <Card className="border-navy-100/30 mb-5">
      <CardHeader>
        <CardTitle className="text-3xl">Performance breakdown</CardTitle>
      </CardHeader>
